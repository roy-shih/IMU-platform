...
//    Serial.println(IMU_data);
  ...
  if (x6 == 1) {
    ...
    if((WiFi.status() == WL_CONNECTED)){
      HTTPClient http;
      http.begin("http://" + server_ip + "/post_api/");
      http.addHeader("Content-Type","application/json");
      int httpResponseCode = http.POST("{\"DEVICE_ID\":\""+Device_ID+"\",\"VALUE\":\""+IMU_data+"\"}");
      Serial.println("{\"DEVICE_ID\":\""+Device_ID+"\",\"VALUE\":\""+IMU_data+"\"}");
...
//      button.tick();
      ...
      if (mySensor.accelUpdate() == 0) {
        ...
        Serial.println("accelX: " + String(G*aX));
        Serial.println("accelY: " + String(G*aY));
        Serial.println("accelZ: " + String(G*aZ));
...
//        Serial.print(gZ);
      ...
      S=String(G*aX)+","+String(G*aY)+","+String(G*aZ)+","+String(gX)+","+String(gY)+","+String(gZ)+";";
